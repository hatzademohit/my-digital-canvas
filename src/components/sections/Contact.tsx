...
const contactSchema = z.object({
  ...
  phone: z.string().trim().min(7, "Phone number too short").max(15, "Phone number too long").regex( /^\+?[1-9]\d{6,14}$/,
...
const contactConfig = {
  SERVICE_ID: "service_72gzs3j",
  TEMPLATE_ID: "template_az1po4m",
  PUBLIC_KEY: "cBQazeXUaNcZE3LDm",
};
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        >
          <p className="text-lg text-muted-foreground">
            Got a project, a role, or just want to chat about React perf?
          ...
          <div className="mt-8 space-y-4">
            {[
              ...
              <a
                key={it.label}
                href={it.href}
                className="flex items-center gap-4 rounded-xl glass p-4 hover:bg-white/10 transition"
              >
        ...
        >
          <Field label="Subject" error={errors.subject?.message}>
            <input
              {...register("subject")}
              className="w-full rounded-lg bg-white/5 border border-white/10 px-4 py-2.5 text-sm outline-none focus:border-primary transition"
          ...
          <Field label="Email" error={errors.email?.message}>
            <input
              ...
              {...register("email")}
              className="w-full rounded-lg bg-white/5 border border-white/10 px-4 py-2.5 text-sm outline-none focus:border-primary transition"
              placeholder="mohithatzade629@gmail.com"
            />
          </Field>
          ...
          <Field label="Message" error={errors.message?.message}>
            <textarea
              rows={5}
              {...register("message")}
              className="w-full rounded-lg bg-white/5 border border-white/10 px-4 py-2.5 text-sm outline-none focus:border-primary transition resize-none"
